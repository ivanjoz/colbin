...
func kOf(buf []byte) uint8         { return (buf[0]>>3)&0x03 + 1 }
func mOf(buf []byte) uint8         { return kOf(buf) + dCodes[buf[0]>>5] }
...
func roundtrip[T Signed](t *testing.T, vals []T) []byte {
	...
	for i := range vals {
		if out[i] != vals[i] {
			t.Fatalf("w=%d idx=%d: got %d want %d (input %v, transform %d, k=%d m=%d)",
				w, i, out[i], vals[i], vals, transformOf(buf), kOf(buf), mOf(buf))
		}
	}
	return buf
}
...
func TestArrayDefinedType(t *testing.T) {
	if got := widthOf[recordID](); got != 4 {
		t.Fatalf("widthOf[recordID] = %d, want 4", got)
	}
...
// wider one: the fixed fallback shrinks with the element width.
...
// The trFixed fallback is always available at 8 bytes, so the encoder can never
// emit more than one header byte plus the uncompressed words.
func TestArrayNeverExceedsFixed(t *testing.T) {
	...
	for range 2000 {
		...
		if len(buf) > 1+8*n {
			t.Fatalf("n=%d: encoded %d bytes, fixed fallback would be %d", n, len(buf), 1+8*n)
		}
	}
}

// The parameter search models sizes from a bit-length histogram. Verify that
// model against real encoding: no other (k, code) pair for the chosen transform
// may produce a shorter payload.
func TestArraySearchIsOptimal(t *testing.T) {
	...
	for range 600 {
		...
		for k := minK; k <= maxK; k++ {
			for code := range uint8(8) {
				...
				fits := true
				for i := range cnt {
					res := residual(vals, i, transform, zz, base)
					if encLen(uint8(bits.Len64(res)), k, m) == 0 {
						fits = false
						break
					}
					alt = appendKM(alt, res, k, m)
				}
				if fits && len(alt)+1 < len(buf) {
					t.Fatalf("n=%d transform=%d: chose k=%d m=%d (%dB) but k=%d m=%d gives %dB\nvals=%v",
						n, transform, kOf(buf), mOf(buf), len(buf), k, m, len(alt)+1, vals)
				}
			}
		}
	}
...
func TestArrayAllTransformsSelected(t *testing.T) {
	corpus := [][]int64{
		...
		{math.MinInt64, math.MaxInt64, 3, 1 << 62, 7, -1 << 62, 11, 1 << 61}, // fixed
	}
	seen := map[uint8]bool{}
	...
	for _, tr := range []uint8{trRaw, trDelta, trFOR, trFixed} {
		if !seen[tr] {
...
// The worked example from the design notes: deltas [100,140,10,130] all fit in
// 8 bits, so k=1/M=1 makes every byte flag-free and each value costs 1 byte.
func TestArrayDeltaExampleIsOptimal(t *testing.T) {
	vals := []int64{100, 240, 250, 380}
	buf := roundtrip(t, vals)
	if got := transformOf(buf); got != trDelta {
		t.Fatalf("transform = %d, want trDelta (%d)", got, trDelta)
	}
	if k, m := kOf(buf), mOf(buf); k != 1 || m != 1 {
		t.Fatalf("params k=%d m=%d, want k=1 m=1", k, m)
	}
	if len(buf) != 5 { // 1 header + 4 single-byte deltas
		t.Fatalf("encoded %d bytes, want 5: % x", len(buf), buf)
	}
	// Plain varint of the same deltas would need 6 bytes (140 and 130 exceed 127).
	if plain := len(appendKM(appendKM(appendKM(appendKM(nil,
		100, 1, 10), 140, 1, 10), 10, 1, 10), 130, 1, 10)); plain != 6 {
		t.Fatalf("plain varint baseline = %d bytes, want 6", plain)
	}
}
...
func fuzzRoundtrip[T Signed](t *testing.T, vals []T) {
	t.Helper()
	buf := AppendArray(nil, vals)
	if len(buf) > 1+len(vals)*int(widthOf[T]()) {
		t.Fatalf("encoded %dB exceeds fixed fallback %dB", len(buf), 1+len(vals)*int(widthOf[T]()))
	}
	out := make([]T, len(vals))
...
func benchEncode[T Signed](b *testing.B, vals []T) {
	...
	b.SetBytes(int64(len(vals)) * int64(widthOf[T]()))
	for b.Loop() {
...
func TestArraySizeReport(t *testing.T) {
	...
	names := [4]string{"raw", "delta", "FOR", "fixed"}
	...
	cases := []struct {
		...
		{"random int64", mk(256, func(i int) int64 { return int64(rr.Uint64()) }), trFixed},
	...
	for _, tc := range cases {
		...
		raw := len(tc.vals) * 8
		got := transformOf(buf)
		t.Logf("%-20s %5dB -> %5dB (%4.1f%%)  transform=%-5s k=%d M=%d",
			tc.name, raw, len(buf), 100*float64(len(buf))/float64(raw),
			names[got], kOf(buf), mOf(buf))
		if got != tc.want {
