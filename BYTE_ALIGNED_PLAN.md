...
| 1 | `BLOB` | `[enc:2][lw:2]` | `[len: lw]` then `len` bytes. `enc`: 0 raw, 1 packed5, 2 dictionary reference, 3 reserved. |
...
| Strings | packed5's ~5 bits/char is gone by default. It survives as `BLOB enc=1`, opt-in, and a dictionary column (§3) beats it outright on the repetitive data packed5 was best at |
