...
//
// The AssemblyScript module asserts against the committed file, so without this
// a change to the Go wire would leave the port passing against a corpus that no
// longer describes anything — which is how the previous port went stale in
// silence. Run `go run ./web/vectors` to regenerate, and expect the module to
...
func TestVectorsAreCurrent(t *testing.T) {
	...
	if string(committed) != string(current)+"\n" {
		t.Fatal("web/vectors/vectors.json is stale: run `go run ./web/vectors`")
	}
}
...
// compose, which is what this asserts — and it is the tier
// REFACTOR_PLAN.md §4.2 is going to change, so it should be the one that is
// checked hardest.
