// Command vectors writes the corpus the browser module is pinned against.
//
...
// # The tiers
//
//	strings   blob framing at both key widths
...
//
// Everything is generated with Packed5 off, which is Go's default. The encoder's
// own documents live in web_encoded.json, written by `bun run emit`.
package main
...
// Wide has an id past fifteen, which — now that the packed string encoding is a
// per-field descriptor code rather than a property of the scope — is the only
// thing that puts a type on the eight-bit key path.
type Wide struct {
	First uint32 `cb:"1"`
	Text  string `cb:"2"`
	Far   int64  `cb:"201"`
...
// the move of the packed encoding went through. The payload is not recorded: it
// is Size copies of Fill, so a 64 KB case costs two fields rather than 128 KB of
// hex, and what is compared is the header alone.
...
type typeCase struct {
	...
	// The guarantee either decoder offers is only safety — a sweep before the
	// port measured that over half of all corruptions decode to well-formed,
	// wrong output, and
...
// `rust/ENCODER.md` §2: a literal with no '.' and no exponent is an integer and must
...
// `rust/ENCODER.md` §5 defines as U+FFFD.
...
//
// The masks are a low bit, a high bit and the whole byte. It is the same sweep
// that ran before the port started, run against the format that exists now
// rather than the one that did then.
...
//
// Neither of these two tiers is about the wire. They pin the module's JSON
// scanner (rust/src/json/parse.rs), which is the reason it parses JSON itself
// instead of taking an object graph from the host: JSON.parse turns
// 7295013456321098765 into 7295013456321098800 before the codec ever sees it.
