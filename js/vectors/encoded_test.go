...
// reproduce and read them. This goes module → Go, which is the direction a
...
func TestGoReadsWhatTheModuleWrites(t *testing.T) {
	...
	if err != nil {
		t.Skipf("no module output to check: %v (run `cd js && node tests/emit.mjs`)", err)
	}
	...
	for _, one := range held.Cases {
		t.Run(one.Name, func(t *testing.T) {
			...
			//
			// Nothing else is compensated for here. The envelope round a
			// non-object top level used to be — Go did not know the section
			// flag and rendered `{"rows": …}` where the module rendered the
			// document — and `codec/json.go`'s `envelopeRun` closed that, so the
			// two now agree outright. A shim for a divergence that no longer
			// exists is a shim that would hide it coming back.
